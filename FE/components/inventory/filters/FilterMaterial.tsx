...
const FilterMaterial: React.FC<any> = () => {
    ...
    const [materialSelected, setMaterialSelected] = useState<any>(filters.color_id);
    const [showBlock, setShowBlock] = useState<boolean>(true);

    const handleitemFilter = (e: any) => {
        if (e.target.checked) {
            setMaterialSelected([...materialSelected, parseInt(e.target.value)]);
            dispatch(
                setPaginationAction({
                    ...
                    modifier: {
                        filters: {
                            ...filters,
                            material_id: [...materialSelected, parseInt(e.target.value)]
                        },
                        offset: 0
                    }
                })
            ...
            dispatch(
                setPaginationAction({
                    ...
                    modifier: {
                        filters: {
                            ...filters,
                            material_id: materialSelected.filter(
            ...
            setMaterialSelected(
                materialSelected.filter((id: any) => id !== parseInt(e.target.value))
            );
        }
    };
                                    ...
                                    <div className="w-1/2" key={item.id}>
                                        <span className="block flex mb-2">
                                            <input
                                                ...
                                                value={item.id}
                                                checked={filters.material_id.includes(item.id)}
